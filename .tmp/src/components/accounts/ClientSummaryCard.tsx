  ...
  return (
    <section className="bg-white border border-[#d0d0d0] shadow-sm rounded-[10px] overflow-hidden mb-4" style={{ fontFamily: "Roboto, system-ui, sans-serif" }}>
      ...
      <div className="p-3">
        ...
        {/* Main Table */}
        <div className="overflow-x-auto border border-[#d5d8dc]">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#00ab81] text-white">
                <th colSpan={9} className="px-3 py-1.5 text-left font-bold text-xs">
                  <button
                  ...
                  return (
                    <tr key={client.id} className={cn(idx % 2 === 0 ? "bg-white" : "bg-[#f9f9f9]", "hover:bg-blue-50/50 transition-colors whitespace-nowrap")}>
                      ...
                      <td className="px-2 py-2 border border-[#d5d8dc] text-right font-black text-[#28a745]">
                        {balancesLoaded ? (client.credit_received || 0).toLocaleString() : "-"}
                      </td>
                      <td className={cn(
                        "px-2 py-2 border border-[#d5d8dc] text-right font-black",
                        balancesLoaded ? (client.cash >= 0 ? "text-[#28a745]" : "text-[#dc3545]") : "text-gray-400"
                      )}>
                        {balancesLoaded ? (client.cash || 0).toLocaleString() : "-"}
                      </td>
                      <td className={cn(
                        "px-2 py-2 border border-[#d5d8dc] text-right font-black",
                        balancesLoaded ? (client.pl_downline >= 0 ? "text-[#28a745]" : "text-[#dc3545]") : "text-gray-400"
                      )}>
                        {balancesLoaded ? (client.pl_downline || 0).toLocaleString() : "-"}
                      </td>
                      <td className="px-2 py-2 border border-[#d5d8dc] text-right font-bold text-[#212529]">
                        {balancesLoaded ? `${client.downline_share || 0}%` : "-"}
                      </td>
                      <td className="px-2 py-2 border border-[#d5d8dc] text-right font-bold text-[#dc3545]">
                        {balancesLoaded ? "0" : "-"}
                      </td>
                      <td className="px-2 py-2 border border-[#d5d8dc] text-right font-black text-[#28a745]">
                        {balancesLoaded ? (client.credit_remaining || 0).toLocaleString() : "-"}
                      </td>
