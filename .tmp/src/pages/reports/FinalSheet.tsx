...
export default function FinalSheet() {
  ...
  const exportColumns = [
    ...
    { key: "cash", label: "Cash" },
    { key: "pl_downline", label: "P&L Downline" },
    { key: "balance_upline", label: "Balance Upline" },
    { key: "net", label: "Net" },
  ];
  ...
  const exportData = (clients || []).map((c, i) => ({
    ...
    username: c.username || "",
    role: c.role || "",
    credit_received: (c.credit_received || 0).toLocaleString(),
    cash: (c.cash || 0).toLocaleString(),
    pl_downline: (c.pl_downline || 0).toLocaleString(),
    balance_upline: (c.balance_upline || 0).toLocaleString(),
    net: ((c.pl_downline || 0) + (c.balance_upline || 0)).toLocaleString(),
  }));
        ...
        <div className="mx-2">
          <section className="bg-white border border-[#d5d8dc] overflow-hidden">
            <div className="bg-[#ecf0f1] border-b border-[#d5d8dc] px-3 py-2 flex items-center gap-2">
              <strong className="text-[13px] text-[#2c3e50]">Final Sheet</strong>
              <ExportButtons 
                data={exportData} 
                columns={exportColumns} 
                filename={`Final-Sheet-${fromDate}-${toDate}`} 
