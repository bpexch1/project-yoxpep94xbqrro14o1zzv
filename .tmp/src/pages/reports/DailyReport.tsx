...
export default function DailyReport() {
  ...
  const { data: bets, isLoading } = useQuery({
    queryKey: ["daily-report", session?.username, fromDate, toDate, usernameFilter, searchTrigger],
    queryFn: async () => {
      if (!session) return [];
      ...
      return all.filter(b => {
        ...
        if (session.role !== 'superadmin' && session.role !== 'company') {
          // In a real app we'd check if user_email belongs to session's downline
          // For now we'll just check if it's their own if they're a client
          if (session.role === 'client') accessMatch = b.user_email === session.username;
        }
        
        return dateMatch && userMatch && accessMatch;
      });
    },
    enabled: !!session,
  });
