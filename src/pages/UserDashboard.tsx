  ...
  const { data: matches = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        return matchesArray.sort((a: any, b: any) =>
          a?.source === "db" ? -1 : 1
        );
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#0b0e14] text-gray-200 font-sans pb-20">
      ...
      <div className="flex gap-2 p-4 overflow-x-auto">
        {["all", "cricket", "soccer", "tennis"].map((sport) => (
          <button
            key={sport}
            onClick={() => setSelectedSport(sport as any)}
            className={cn(
              "px-4 py-2 rounded text-[10px] font-black uppercase whitespace-nowrap",
              selectedSport === sport
        ...
        {isLoading ? (
          ...
          filteredMatches.map((match: any) => (
            <div
              ...
              onClick={() =>
                navigate(`/play/match/${match?._id || match?.id}`)
              }
              className="bg-[#161b26] border border-gray-800 rounded-lg p-3 cursor-pointer"
            >
              <div className="flex justify-between mb-2">
                <span className="text-[9px] font-black uppercase text-gray-500 flex items-center gap-1">
                  ...
                  {match?.sport_type || "Unknown"}
                </span>

                {match?.source === "db" && (
                  <span className="text-[8px] bg-blue-500/20 text-blue-400 px-1 rounded font-black">
                    ADMIN CONTROLLED
                  </span>
                )}
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-1">
                  <div className="bg-[#1a2d47] p-2 rounded text-center min-w-[60px]">
                  ...
                  <div className="bg-[#372335] p-2 rounded text-center min-w-[60px]">
                    <div className="text-[8px] text-pink-400">
                      LAY
                    </div>
                    <div className="text-xs font-black">
                      {match?.lay_odds_a ?? "-"}
