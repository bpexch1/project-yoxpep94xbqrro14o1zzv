...
export default function BookDetail() {
  ...
  const { data: transactions, isLoading } = useQuery({
    queryKey: ["transactions", session?.username, fromDate, toDate, searchTrigger],
    queryFn: async () => {
      if (!session) return [];
      
      // We filter by date range manually after fetching because we want running balance correctly if possible, 
      // but for simple ledger we'll just filter by date range.
      let query = TransactionEntity.query().sort("-created_at");
      
      if (session.role !== 'superadmin' && session.role !== 'company') {
        query = query.where("client_username", session.username);
      }
