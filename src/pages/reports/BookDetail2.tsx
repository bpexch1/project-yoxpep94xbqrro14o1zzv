...
export default function BookDetail2() {
  ...
  const { data: bets, isLoading } = useQuery({
    queryKey: ["bets-detail-2", session?.username, fromDate, toDate, searchTrigger],
    queryFn: async () => {
      if (!session) return [];
      
      let query = BetEntity.query().sort("-created_at");
      
      // If not superadmin/company, only see their own bets (or downline, but usually book is for user)
